...
const limpiar = () => { filtro.q = ''; filtro.facultad_id = ''; filtro.estado = ''; router.get('/estructura/programas', {}, { preserveScroll: true, replace: true }); };
const cambiarEstado = (p) => router.patch(`/estructura/programas/${p.id}/estado`, {}, { preserveScroll: true });
const eliminar = (p) => { if (confirm(`¿Eliminar el programa "${p.nombre}"?`)) router.delete(`/estructura/programas/${p.id}`, { preserveScroll: true }); };
</script>
    ...
    <div>
        <div class="mb-6 flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-semibold text-[#1F3864]">Programas Académicos</h1>
                <p class="mt-1 text-sm text-slate-500">Carreras o programas que ofrece cada facultad.</p>
            </div>
            <Link href="/estructura/programas/crear"
                ...
                <span class="text-base leading-none">+</span> Nuevo programa
            </Link>
        </div>
        ...
        <div class="mb-6 grid gap-4 lg:grid-cols-[1fr_auto]">
            ...
            <div class="flex min-w-[160px] flex-col justify-center rounded-xl bg-gradient-to-br from-[#1F3864] to-[#2E75B6] p-5 text-white shadow-sm">
                <span class="text-xs font-medium uppercase tracking-wide text-blue-100">Programas activos</span>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="p in programas.data" :key="p.id" class="hover:bg-slate-50/70">
                            <td class="px-4 py-3 font-mono text-xs text-slate-600">{{ p.codigo }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ p.facultad || '—' }}</td>
                            <td class="px-4 py-3 font-medium text-slate-800">{{ p.nombre }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ p.planes_count }}</td>
                            <td class="px-4 py-3">
                        ...
                        <tr v-if="!programas.data.length">
                            <td colspan="6" class="px-4 py-10 text-center text-slate-400">No se encontraron programas académicos.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div v-if="programas.data.length" class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3">
                <p class="text-xs text-slate-500">Mostrando {{ programas.from }}–{{ programas.to }} de {{ programas.total }}</p>
